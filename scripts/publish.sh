...
check_npm_auth() {
    # Check if user is already logged in to npm
    if npm whoami > /dev/null 2>&1; then
        echo "✅ Already authenticated with npm"
        return 0
    fi
    ...
    if [ -n "$NPM_PUBLISH_TOKEN" ]; then
        ...
        if npm whoami > /dev/null 2>&1; then
            echo "✅ NPM_PUBLISH_TOKEN authentication successful"
            return 0
        else
            echo "❌ NPM_PUBLISH_TOKEN is invalid or expired"
        fi
    fi
