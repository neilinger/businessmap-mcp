...
const workflows = Array.isArray(boardStructure.workflows) ? boardStructure.workflows : [];
if (workflows.length < 1) {
  throw new Error('Board must have at least one workflow for testing');
}
```
...
it('[CONCURRENT-002] Concurrent moveCard maintains parent links', async () => {
  ...
  const movePromises = childIds.map((id) => client.moveCard(id, newColumn));
